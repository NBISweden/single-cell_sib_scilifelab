...
We get two matrices here, for both spliced and unspliced counts. Note that these are loaded in as sparse matrices.
...
The kallisto/BUStools pipeline does not do any filtering on the features or cells, so it makes sense that most droplets contain only a very tiny number of UMIs.
...
In this sample, it is clear that a sizeable group of cells has large spliced UMI counts (1000 - 10000). Note that although the total UMI counts are different between spliced and unspliced counts, the y location of the inflection point does not change that much. Moreover, the same empty droplets are retained in both analyses:
...
The proportion of unspliced reads is typically around 20%, irregardless of the technology. This was already described in [@mannoRNAVelocitySingle2018]. Although slight deviations between 10% and 50% are quite common (especially outside of mammals), extreme values might indicate that something went wrong in the detection of unspliced reads.
...
Let's do a basic log2 and size factors normalization. SCTransform [@hafemeisterNormalizationVarianceStabilization2019] can also be used here, but for the sake of 
...
Do you see the trajectory appearing?
...
```{r}
write_rds(seu, "../data/adipose_differentiation-merick/seu_trajectory.rds")
